...
module.exports = {
  information: {
    'resolve duplicates': {
      title: 'Resolve {} possible duplicate(s)',
      check: function (person) {
        ...
        person.more.duplicates.map(function (dup) {
          links[dup.title] = 'https://familysearch.org/tree/#view=merge&person=' + person.rels.id + '&otherPerson=' + dup.id
        })
        return {
          args: [person.more.duplicates.length],
          links: links
        }
      }
    },
  ...
  relationships: {
    ...
    'find children': {
      title: 'Find more children (only {} recorded)',
      check: function (person) {
        if (person.rels.display.lifespan.match(/Living/)) return false
        var range = ageRange(person.rels.display.lifespan)
