...
Db.prototype = {
  ...
  addHistoryItem: function (id, key, value, done) {
    this.db.collection('history').insert({
      id: id,
      key: key,
      user: this.userId,
      value: value
  ...
  addTodoHistoryItem: function (id, type, attr, val, done) {
    this.db.collection('history').insert({
      id: id,
      key: attr,
      todo: type,
  ...
  setTodoAttr: function (id, type, attr, val, done) {
    var obj = {}
    obj['todos.$.' + attr] = val
  ...
  setGetTodoAttr: function (id, type, attr, val, done) {
    this.setTodoAttr(id, type, attr, val, function (err) {
      if (!done) return
      if (err) return done(err)
      async.parallel({
        person: this.getData.bind(this, id),
