...
function duplicates(id, matches, nots) {
  ...
  fs.writeFileSync('ex-dubplicates.json', JSON.stringify(matches))
  fs.writeFileSync('ex-matches.json', JSON.stringify(nots))
  var ids = nots.persons.map(function (person) {
    return person.id
  })
  return matches.entries.filter(function (entry) {
    return entry.gedcomx.persons.some(function (person) {
      return ids.indexOf(person.id) !== -1
    })
  }).map(function (dup) {
    return {
      score: dup.score,
